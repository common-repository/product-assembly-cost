...
class WC_Product_Extra_Service_Assembly {
	...
	private $default_cost;
	private $cost_multiply;
	private $message;
	...
	public function assembly_option_html() {
		...
		if ( $product->get_type() != 'variable' ) {
			...
			if ( apply_filters( 'product_assembly_show_assembly', $show_assembly, $product ) ) {
				...
				if ( floatval( $cost ) === floatval( 0 ) ) $cost = $this->default_cost; // Default.
			...
			foreach ( $variations as $variation ) {
				...
				if ( floatval( $cost ) === floatval( 0 ) ) $cost   = apply_filters( 'product_assembly_cost', floatval( $product->get_meta( '_assembly_cost' ) ), $product ); // Backwards compatibility - get from product
				if ( floatval( $cost ) === floatval( 0 ) ) $cost   = $this->default_cost; // Default
				$cost                     = $this->show_cost_with_or_without_vat( $cost, $product );
				$show_assembly            = 0;
				if ( $this->enabled_default ) {
	...
	public function get_cost_for_cart_item( $cart_item, $display = false ) {
		...
		if ( $cost === '' || floatval( $cost ) === floatval( 0 ) ) { // Default
